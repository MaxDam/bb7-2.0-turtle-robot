...
class BB7Motion:
	...
	def happy(self):
		...
		bb7.armBackRight(60).armBackRight(60).armFrontRight(0).armFrontLeft(0).head(10).send();
		time.sleep(0.4)

		#nuove il dietro come un cane
		for degree in [30, -30, 30, -30, 30, -30, 30, -30, 0]:
			bb7.shoulderBackRight(-degree).shoulderBackLeft(degree).send();
			time.sleep(0.2)
	...
	def stepForward(self, stepCount=0, delay=0.1, arm_zero_pos=50):
		bb7.zero(arm_zero_pos)
		time.sleep(delay)
		#conta i passi in avanti
		for i in range(stepCount):
			#porta il peso indietro a sinistra
			bb7.armBackRight(arm_zero_pos-50).armBackLeft(arm_zero_pos-70).send()
			time.sleep(delay)
			#muove a gamba anteriore destra per il passo
			bb7.armFrontRight(-40).send()
			time.sleep(delay)
			...
			bb7.armFrontRight(arm_zero_pos-90).armFrontLeft(arm_zero_pos-20).send()
			time.sleep(delay)
			#muove a gamba posteriore sinistra per il passo
			bb7.armBackLeft(-40).send()
			time.sleep(delay)
			bb7.shoulderBackLeft(40).send()
			time.sleep(delay)
			#reimposta l'assetto gambe
			...
			bb7.armBackRight(arm_zero_pos-70).armBackLeft(arm_zero_pos-50).send()
			time.sleep(delay)
			#muove a gamba anteriore sinistra per il passo
			bb7.armFrontLeft(-40).send()
			time.sleep(delay)
	...
	def stepBack(self, stepCount=0, delay=0.1, arm_zero_pos=50):
		...
		for i in range(stepCount):
			...
			#porta il peso in avanti a destra
			bb7.armFrontRight(arm_zero_pos-70).armFrontLeft(arm_zero_pos-50).send()
			time.sleep(delay)
			#muove a gamba posteriore sinistra per il passo
			bb7.armBackLeft(-40).send()
			time.sleep(delay)
	...
	def stepTurnLeft(self, stepCount=0, delay=0.1, arm_zero_pos=50):
		...
		for i in range(stepCount):
			...
			time.sleep(delay)
			#reimposta l'assetto gambe
			bb7.armFrontLeft(arm_zero_pos).armBackRight(arm_zero_pos).armBackLeft(arm_zero_pos).send()
			time.sleep(delay)
			#porta il peso in avanti a sinistra
			bb7.armFrontRight(arm_zero_pos-50).armFrontLeft(arm_zero_pos-70).send()
			time.sleep(delay)
